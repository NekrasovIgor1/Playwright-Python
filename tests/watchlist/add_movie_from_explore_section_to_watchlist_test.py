...
# add video from main page - "Explore what’s streaming" section ,to watchlist
@pytest.mark.usefixtures("setup_page_class")

class TestAddMovieFromExploreSectionToWatchlist(BaseTest):
    def test_add_movie_from_explore_section_to_watchlist(self):
        self.perform_login()
        # add the first movie (index 1-10) from the Top picks section to the watchlist,
        # and get the movie title for validation
        movie_name_added = self.main_page.add_movie_to_watchlist_by_index_from_explore_section(2)
